...
function getProggressbar (percentage) {
  let proggressbar = '';
  let steps = 100;
  let target = parseInt((steps*percentage) / 100);
...
function showProggress (actionName, percentage) {
  let proggressbar = getProggressbar(percentage);
  process.stdout.write(proggressbar + ' ' +  percentage + '% complete... (' + actionName + ')\r');
}
...
var config = {
  ...
  remoteRoot: '/domains/matiaan.com/public_html/app/front',
...
ftpDeploy.on('uploading', function(data) {
  ...
  showProggress(data.filename, Math.floor((data.transferredFileCount/data.totalFilesCount) * 100))
});
